...
dedicated support in the TMT application. Namely, the passphrase encoding type 
...
in passphrase in Keepkey will result in different Terracoin addresses than those that would 
generate Trezor for the same seed.

This issue results in confusion, especially when exchanging devices between KeepKey and 
those, that conform the BIP-39 standard. To help in such cases, TMT gives you control over 
...
Keep in mind that the choice of encoding type affects the Terracoin addresses generated for the 
...
between TMT and the KeepKey official client app, use NFC. 
