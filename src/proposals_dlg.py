...
from crownd_intf import CrowndIndexException, Masternode
...
class Proposal(AttrsProtected):
    ...
    def apply_values(self, masternodes, last_superblock_time, next_superblock_datetime):
        ...
        if not self.get_value('title'):
            # if title value is not set (it's an external attribute, from crowncentral) then copy value from the
            # name column
...
class ProposalsDlg(QDialog, ui_proposals.Ui_ProposalsDlg, wnd_utils.WndUtils):
        ...
        for idx, mn in enumerate(self.app_config.masternodes):
            ...
            if mn_ident not in mn_idents:
                if mn.dmn_voting_key_type == InputKeyType.PRIVATE:
                    voting_key = mn.get_current_key_for_voting(self.app_config, self.crownd_intf)
                    if voting_key:
                        if crown_utils.validate_wif_privkey(voting_key, self.app_config.crown_network):
    ...
    def display_budget_summary(self):
        ...
        if self.next_budget_approved is not None:
            budget_approved = f'<td><b>Budget approved:</b> {app_utils.to_string(round(self.next_budget_approved))} Crown '
        ...
        if self.next_budget_requested is not None:
            budget_requested = f'<td><b>Budget requested:</b> {app_utils.to_string(round(self.next_budget_requested))} Crown '
        ...
        if self.votes_loaded:
            if self.next_budget_approved_by_user_yes_votes is not None:
                budget_approved_user_yes = \
                    ...
                    f'{app_utils.to_string(round(self.next_budget_approved_by_user_yes_votes))} Crown '
                if self.next_budget_amount:
                    budget_approved_user_yes += \
        ...
        message = '<html><head></head><style>td{padding-right:10px;white-space:nowrap;}</style><body>' \
                  ...
                  f'{bra}<tr><td><b>Budget available:</b> {app_utils.to_string(round(self.next_budget_amount))} Crown</td>' \
                  f'<td><b>Budget left:</b> {app_utils.to_string(round(self.next_budget_left))} Crown</td></tr>' \
    ...
    def read_proposals_from_network(self):
        ...
        try:
            ...
            if len(proposals_new) > 0:
                        ...
                        for prop in self.proposals:
                            ...
                            if prop.marker:
                                if not prop.db_id:
                                    ...
                                    # crownd sometimes does not return some proposals, so they are deactivated id the db
                ...
                log.warning('No proposals returned from crownd.')
            ...
            log.exception('Exception wile reading proposals from Crown network.')
            self.display_message('')
    ...
    def get_block_timestamp(self, superblock: int):
        ...
        if ts is None:
            bhash = self.crownd_intf.getblockhash(superblock)
            bh = self.crownd_intf.getblockheader(bhash)
            ts = bh['time']
        ...
        try:
            ...
            if not self.crownd_intf.open():
                ...
                try:
                    ...
                    mns = self.crownd_intf.get_masternodelist('json')
            ...
            if not self.finishing:
                # read additional data from external sources, if configured (CrownCentral)
                proposals = []
    ...
    def read_external_attibutes(self, proposals):
        """Method reads additional proposal attributes from an external source such as CrownCentral.org/CrownNexus.org
        ...
        try:
            url = self.app_config.crown_central_proposal_api
            if url:
            ...
            try:
                ...
                if not self.crownd_intf.open():
                    ...
                    try:
                        ...
                        node_info = self.crownd_intf.rpc_call(False, False, 'getinfo')
    ...
    def refresh_proposals_thread(self, ctrl):
        ...
        proposals = []
        if self.app_config.read_proposals_external_attributes:
            # select proposals for which we read additional data from external sources as CrownCentral.org
            for prop in self.proposals:
                if not prop.ext_attributes_loaded:
                    proposals.append(prop)
        if proposals and not self.finishing:
    ...
    def refresh_details_tabs(self):
        try:
            ...
            if self.current_proposal:
                ...
                if not re.match('.*crowncentral\.org', url.lower()):
                    dc_url = 'https://www.crowncentral.org/p/' + prop.get_value('name')
...
<table>
    <tbody>
        ...
        <tr class="main-row">
            ...
            <td class="padding" style="white-space:nowrap"><span>{app_utils.to_string(prop.get_value('payment_amount'))} Crown&#47;cycle ({cycles_str}, {app_utils.to_string(prop.get_value('payment_amount_total'))} Crown total)
...
<table>
    <tbody>
        ...
        <tr class="main-row">
            ...
            <td class="padding">{app_utils.to_string(total_amount_requested)} Crown{total_pct_requested}</td>
        </tr>
        <tr class="main-row">
            <td class="first-col-label">Approved from the next budget:</td>
            <td class="padding">{app_utils.to_string(total_amount_approved)} Crown{total_pct_approved}</td>
        </tr>
    </tbody>
</table>
 </body>
        ...
        for prop in proposal_list:
            ...
            for vote_idx, mn_info in enumerate(masternodes):
                ...
                try:
                    ...
                    step = 2
                    vote_sig = crown_utils.ecdsa_sign(
                        serialize_for_sig,
                        mn_info.masternode_config.get_current_key_for_voting(self.app_config, self.crownd_intf),
                    ...
                    if step in (1, 4):
                        ...
                        log.info('masternode_pub_key: %s' %
                                     str(crown_utils.wif_privkey_to_pubkey(
                                         mn_info.masternode_config.get_current_key_for_voting(
                                             self.app_config, self.crownd_intf))))
                        log.info('masternode_pub_key_hash: %s' %
                                     str(crown_utils.pubkey_to_address(crown_utils.wif_privkey_to_pubkey(
                                         mn_info.masternode_config.get_current_key_for_voting(
                                             self.app_config, self.crownd_intf)), self.app_config.crown_network)))
